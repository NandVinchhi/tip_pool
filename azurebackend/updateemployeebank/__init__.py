...
def login_employee(email, password, db):
...
def update_bank_details_employee(email, password, bank_number, db):
    if login_employee(email, password, db)["status"] == "success":
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if req.get_json():
        ...
        data = req.get_json()
        ret = json.dumps(update_bank_details_employee(data["email"], data["password"], data["bank_number"], db))
